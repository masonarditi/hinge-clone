-- 20250522000000_update_get_profiles_function.sql

-- Drop the old function first
DROP FUNCTION IF EXISTS public.get_profiles(integer);

-- Recreate with proper column types
...
select
  ...
  p.user_role,  -- Moved position to match return order
  (
    select coalesce(jsonb_agg(json_build_object(
